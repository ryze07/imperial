let products = JSON.parse(localStorage.getItem("products")) || [];
        ...
        function createProduct(event) {
            ...
            if (products.some((product) => product.name === name)) {
                alert("Já existe um produto com este nome. Por favor, escolha um nome diferente.");
                return;
            }
        ...
        function updateProduct(event) {
            ...
            const newDescription = document.getElementById("new-product-description").value;
            ...
            if (products.some((p) => p.name === newName && p.id !== product.id)) {
                alert("Já existe um produto com este novo nome. Por favor, escolha um nome diferente.");
                return;
            }
            ...
            if (products.length > 0) {
                products.forEach((product) => {
                    productList.innerHTML += `
                        <div>
                            <img src="${product.image}" alt="${product.name}" style="max-width: 200px; border-radius: 5px;">
                            <p>Nome: ${product.name}</p>
                            <p>Marca: ${product.brand}</p>
